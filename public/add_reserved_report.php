...
<head>
    <meta charset="UTF-8">
    <title>طلب حجز حيوان للتبني</title>
    <style>
        :root {
        ...
        /* قسم الملاحظات - تحسين التنسيق */
        .notes {
            background-color: var(--light-beige);
            border: 1px solid var(--olive-tone);
            padding: 8px;
            margin: 10px 0;
            border-radius: 4px;
        }
        .notes h3 {
            color: var(--main-green);
            text-align: center;
            border-bottom: 1px solid var(--olive-tone);
            padding-bottom: 3px;
            margin-bottom: 8px;
            font-size: 13px;
        }
        .notes ul { font-size: 9px; line-height: 1.3; list-style-type: square; padding-right: 15px; margin-top: 3px; }
        .notes .disclaimer-note { font-size: 8px; margin-top: 5px; text-align: center; color: var(--olive-tone); font-style: italic; }
        ...
        @media print {
            ...
            .no-print { display: none !important; }
            .header, .report-body, .notes, .signatures { page-break-inside: avoid; }
        }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="header-title">
                <h1>طلب حجز حيوان للتبني</h1>
                <p style="font-size: 12px; margin: 0; color: var(--main-green);">Animal Adoption Reservation Request</p>
                </div>
        ...
        <div class="notes" id="notesSection" style="display: none;">
            <h3>ملاحظات مهمة للحجز / Important Notes for Booking</h3>
            <div class="report-body" style="grid-template-columns: 1fr 1fr; gap: 5px; margin-bottom: 0; border: none; padding: 0;">
                <div class="column arabic" style="border: 1px solid var(--border-color); padding: 5px;">
                    <ul>
                        <li>يتم حجز الحيوان (قط/كلب) بعد دفع رسوم التبني.</li>
                        <li>يتم حجز الحيوان لمدة 4 أيام عمل فور جاهزيته للتبني.</li>
                        <li>يتم إلغاء طلب الحجز في حال عدم الرد من مقدم الطلب خلال قنوات التواصل عن طريق الهاتف، الواتس آب أو البريد الإلكتروني.</li>
                    </ul>
                    <p class="disclaimer-note">هذا نموذج حجز ولا يعتبر تبنيًا</p>
                </div>
                <div class="column english" style="border: 1px solid var(--border-color); padding: 5px;">
                    <ul>
                        <li>This reservation will be after paying adoption fees.</li>
                        <li>This reservation will be for 4 working days once the animal is ready for adoption.</li>
                        <li>This reservation will be canceled if there is no response from the applicant via phone call, WhatsApp, or email.</li>
                    </ul>
                    <p class="disclaimer-note">This is a reservation form and does not constitute adoption</p>
                </div>
            </div>
        </div>
        <div class="signatures" id="signaturesSection" style="display: none;">
             ...
             <div class="registrar-info">
                 ...
                 <p style="margin: 5px 0 0 0; font-size: 9px; color: var(--olive-tone); text-align: center;"> (تم تسجيل الحجز من قبل الموظف المذكور - لا يتطلب توقيع)</p>
            </div>
        
            <div class="signature-box">
                <p class="signature-label" style="font-size: 11px; color: var(--main-green); margin-top: 0;">توقيع مقدم الطلب / Applicant's Signature</p>
                <div style="flex-grow: 1; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                    <img id="adopterSignatureImg" class="signature-img" alt="توقيع مقدم الطلب" style="display: none;">
                ...
                <p class="signature-label" style="margin-bottom: 0;">اسم مقدم الطلب / Applicant's Name</p>
                <p id="adopterSignatureName" style="font-weight: bold; font-size: 10px; color: var(--text-color); margin-top: 1px;"></p>
            </div>
        </div>
        <div class="official-stamp">
            <p>مستند حجز رسمي صادر عن مأوى الشارقة للقطط والكلاب</p>
            <p style="margin: 0;">للاستفسارات: أرضي 06-5453054 | هاتف 056-3299669 واتس آب 056-3299669 | مركز الاتصال 993 | بريد إلكتروني info@shjmun.gov.ae</p>
        </div>
    </div>
<script src="session_check.js"></script>
    <script>
        ...
        const apiUrl = '/health_vet/api/add_adopter.php'; // لا تستخدم مسار نسبي مثل ../ إذا لم تكن متأكداً
        ...
        if (adoptionId) {
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        const adoptionArabicRows = `
                            <tr><td class="label">تاريخ الحجز:</td><td>${data.adoption_date || 'غير محدد'}</td></tr>
                        ...
                        const adoptionEnglishRows = `
                            <tr><td class="label">Reservation Date:</td><td>${data.adoption_date || 'N/A'}</td></tr>
                        ...
                        document.getElementById('reportContent').innerHTML = `
                            <div class="report-body">
                                <div class="column arabic">
                                    <div class="section">
                                        <h2 style="color: var(--main-green);">تفاصيل الحجز</h2>
                                        <table>
                                            ${adoptionArabicRows}
                                        </table>
                                    </div>
                                    <div class="section">
                                        <h2 style="color: var(--main-green);">بيانات مقدم الطلب</h2>
                                        <table>
                                            ${adopterArabicRows}
                                        </table>
                                    </div>
                                ...
                                <div class="column english">
                                     <div class="section">
                                        <h2 style="color: var(--main-green);">Reservation Details</h2>
                                        <table>
                                            ${adoptionEnglishRows}
                                        </table>
                                    </div>
                                    <div class="section">
                                        <h2 style="color: var(--main-green);">Applicant's Details</h2>
                                        <table>
                                            ${adopterEnglishRows}
                                        </table>
                                    </div>
                        ...
                        if (data.adopter_signature) {
                            ...
                            adopterSignatureImg.style.display = 'inline-block';
            ...
            document.getElementById('reportContent').innerHTML = '<p style="color: var(--main-green); padding: 20px;">معرف الحجز مفقود.</p>';
        }
    </script>
</body>
</html>